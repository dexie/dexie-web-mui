...
_Reference: [Table.filter()](</docs/Table/Table.filter()>)_
...
_News in Dexie 2.0.0-beta.6: You can now wait for other async APIs and still hold the transaction active, using [Dexie.waitFor()](</docs/Dexie/Dexie.waitFor()>)_
