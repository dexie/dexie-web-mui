...
The add-ons [Dexie.Observable.js](https://old.dexie.org/docs/Observable/Dexie.Observable.js) and [Dexie.Syncable.js](https://old.dexie.org/docs/Syncable/Dexie.Syncable.js) use `hook('creating')`, `hook('updating')` and `hook('deleting')` to make the database locally observable as well as syncable with a remote server.
