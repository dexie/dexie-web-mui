...
If the target property does not exist, or is null or undefined, the target value will set to the result of computing `0 - argument` or `0n - argument` if argument is a BigInt.
...
If the argument is an array, `remove()` will remove the given items in the argument array from the target array if item exists in the array.
