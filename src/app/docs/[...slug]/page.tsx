...
export async function generateMetadata({ params }: DocPageProps) {
  ...
  const createDescription = (title: string): string => {
    ...
    const keywordMap: Record<string, string> = {
      'Tutorial': 'Learn Dexie.js step-by-step with this complete tutorial. Build offline-first web applications using IndexedDB wrapper with React, Vue, Angular examples.',
      'API-Reference': 'Complete Dexie.js API reference. All methods, properties, and examples for building offline-first applications with IndexedDB.',
      'liveQuery': 'Dexie.js liveQuery() enables reactive database queries. Automatically update React, Vue, Angular components when IndexedDB data changes.',
      'Dexie.js': 'Dexie.js main documentation. Learn how to use the powerful IndexedDB wrapper for offline-first web applications.',
      'Transaction': 'Dexie.js transactions for safe database operations. Learn atomic operations, error handling, and best practices for IndexedDB.',
      'Table': 'Dexie.js Table API reference. Methods for querying, inserting, updating and deleting data in IndexedDB with ease.',
      'Collection': 'Dexie.js Collection API for advanced querying. Filter, sort, and manipulate IndexedDB data with powerful collection methods.',
      'WhereClause': 'Dexie.js WhereClause for precise queries. Learn indexing, filtering, and performance optimization for IndexedDB applications.',
    }
  ...
  const createKeywords = (title: string, slugString: string): string[] => {
    ...
    if (slugString.includes('cloud')) {
      contextKeywords.push('dexie cloud', 'offline sync', 'real time sync', 'database sync')
    }
    ...
    if (slugString.includes('angular')) {
      contextKeywords.push('angular offline', 'angular indexeddb', 'angular database')
    }
    if (slugString.includes('tutorial')) {
      contextKeywords.push('offline tutorial', 'indexeddb tutorial', 'local storage tutorial')
    }
    ...
    if (title.toLowerCase().includes('livequery')) {
      contextKeywords.push('reactive queries', 'live queries', 'real time updates')
    }
    if (title.toLowerCase().includes('transaction')) {
      contextKeywords.push('database transactions', 'atomic operations', 'data consistency')
    }
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${doc.metadata.title} - Dexie.js Documentation`,
        },
      ],
    },
    twitter: {
