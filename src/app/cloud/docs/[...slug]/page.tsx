...
export async function generateMetadata({ params }: CloudDocPageProps) {
  ...
  const createCloudDescription = (title: string, slug: string): string => {
    const cloudKeywordMap: Record<string, string> = {
      'authentication': 'Dexie Cloud authentication guide. Implement secure user login, OTP authentication, and OAuth integration for offline-first applications with zero backend setup.',
      ...
      'permissions': 'Dexie Cloud permissions and access control. Manage user roles, data sharing, and security for multi-user offline applications.',
      'db.cloud': 'Dexie Cloud API reference. Complete documentation for cloud configuration, sync methods, and real-time collaboration features.',
      'access-control': 'Dexie Cloud access control system. Implement fine-grained permissions, user roles, and secure data sharing in offline-first apps.',
      'realms': 'Dexie Cloud realms for data isolation. Organize and secure application data with multi-tenant architecture and selective sharing.',
      'members': 'Dexie Cloud member management. Handle team collaboration, user invitations, and member permissions in offline-first applications.',
      'roles': 'Dexie Cloud role-based access control. Define user permissions, manage team roles, and secure collaborative features.',
      'getting-started': 'Get started with Dexie Cloud. Step-by-step setup guide for adding real-time sync and authentication to offline-first applications.',
      'best-practices': 'Dexie Cloud best practices. Performance optimization, security guidelines, and architecture patterns for production offline-first apps.',
    }

    // Hitta matchande beskrivning
    for (const [key, desc] of Object.entries(cloudKeywordMap)) {
      if (title.toLowerCase().includes(key) || slug.toLowerCase().includes(key)) {
        return desc
      }
    }
    ...
    } else if (slug.includes('tutorial') || title.toLowerCase().includes('tutorial')) {
    ...
    } else if (slug.includes('guide') || title.toLowerCase().includes('guide')) {
  ...
  const createCloudKeywords = (title: string, slug: string): string[] => {
    ...
    if (slug.includes('auth') || title.toLowerCase().includes('auth')) {
      contextKeywords.push('offline auth', 'user authentication', 'login system', 'otp authentication')
    }
    if (slug.includes('sync') || title.toLowerCase().includes('sync')) {
      contextKeywords.push('data sync', 'real time database', 'conflict resolution', 'offline sync')
    }
    if (slug.includes('permission') || title.toLowerCase().includes('permission')) {
      contextKeywords.push('access control', 'user permissions', 'data security', 'role based access')
    }
    if (slug.includes('collaboration') || title.toLowerCase().includes('collaboration')) {
      contextKeywords.push('team collaboration', 'shared data', 'collaborative editing', 'multi user')
    }
    if (slug.includes('api')) {
      contextKeywords.push('cloud api', 'sync api', 'javascript api', 'database api')
    }
    if (slug.includes('getting-started') || slug.includes('tutorial')) {
      contextKeywords.push('cloud tutorial', 'sync tutorial', 'offline first tutorial')
    }
