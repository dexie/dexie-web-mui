...
export function extractFullTextTokens(
  text: string,
  minChars = 2
): Map<string, number> {
  ...
  // Pre-compile regex and keyword set for better performance
  ...
  const tokens = text
    ...
    .reduce<string[]>((acc, token) => {
      ...
      if (token.length < minChars) return acc;

      // Skip single/double digits and programming keywords
      if (/^\d{1,2}$/.test(token) || programmingKeywords.has(token)) return acc;

      // Clean up punctuation and apply stemming
      const cleaned = token.replace(/^[^\w]+|[^\w]+$/g, "");
      if (cleaned.length < minChars) return acc;

      const stemmed = simpleStem(cleaned);
      const finalToken = stemmed.length > 20 ? stemmed.slice(0, 20) : stemmed;

      // Final check: length and stop words
      if (finalToken.length >= minChars && !stopWords.has(finalToken)) {
        acc.push(finalToken);
      }
